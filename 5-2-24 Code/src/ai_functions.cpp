...
void moveToPosition(double target_x, double target_y, double target_theta = -1, int Dspeed = 75, int Tspeed = 75, bool frontfacing = true) 
{
    ...
    float targetThreshold = 7; // represnts the radius of the current postion if target point lies within the circle then move to postion function will end
...
void getObject(int type)
{
    ...
    if (Balldetect.hue() > 90 && Balldetect.hue() < 110)
        Intake.stop(coast);
}
